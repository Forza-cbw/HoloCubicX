...
#include <NTPClient.h>
// NTP 服务器信息
static const char* ntpServer = "ntp.aliyun.com"; // 阿里云NTP服务器
static const long  gmtOffset_sec = 8 * 3600;     // 中国时区（UTC+8）
static const int   daylightOffset_sec = 0;       // 无需夏令时偏移
// 使用 UDP 连接
static WiFiUDP ntpUDP;
// 创建NTP客户端
NTPClient timeClient_old(ntpUDP, ntpServer, gmtOffset_sec, 60000); // 60秒同步一次
#define USE_NTP //开启NTP时间获取,屏蔽这个就会用淘宝api
...
static long long getTimestamp(String url)
{
    ...
    {
        // 初始化 NTP 客户端
        timeClient_old.begin();
        Serial.println("timeClient_old.begin();");
    }

    // while(1)
    // {
        // 更新 NTP 时间
        timeClient_old.update();
        // 获取当前时间戳
        unsigned long long epochTime = timeClient_old.getEpochTime();

         // 将时间戳转换为本地时间（加上时区偏移）
        unsigned long long localTime = epochTime + gmtOffset_sec;
        // unsigned long long localTime = epochTime;//这里不加偏移了，后面有单独算偏移的
        Serial.print("本地时间戳 (UTC+8): ");
        ...
        run_data->m_preNetTimestamp = epochTime*1000 + run_data->m_errorNetTimestamp;   //秒的时间戳变ms的
        run_data->m_preLocalTimestamp = GET_SYS_MILLIS();
        Serial.print("run_data->m_preNetTimestamp=");
        Serial.println(run_data->m_preNetTimestamp);
                            ...
                            const ImuAction *act_info)
{
    ...
    if (1 == run_data->clock_page) // 更新时钟
    {
        ...
        long long timestamp = getTimestamp(); // nowapi时间API
