                                 ...
                                 const ImuAction *action)
{
    ...
    else if (1 == run_data->tcp_start)
    {
        ...
        if (ss_client.connected())
        {
            ...
            if (ss_client.available())
            {
                ...
                if (true == get_mjpeg_ret)
                {
                    ...
                    log_i("帧大小：%d ", frame_size);
                    log_i("MCU处理速度：%.2f FPS",1000.0 / (GET_SYS_MILLIS() - deal_time));
        ...
        else
        {
            ...
            if (ss_client.connected())
            {
                log_i("Controller was connected!");
                ss_client.write("ok"); // 向上位机发送下一帧发送指令
            }
            ...
            while (ss_client.available() == 0)
            {
                ...
                {
                    log_i("Controller was disconnect!");
                                  ...
                                  void *ext_info)
{
    ...
    {
        log_i("APP_MESSAGE_WIFI_CONN enable\n");
        display_screen_share(
            "Screen Share",
            WiFi.localIP().toString().c_str(),
            "8081",
